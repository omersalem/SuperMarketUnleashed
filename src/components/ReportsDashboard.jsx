...
const ReportsDashboard = ({
  ...
  // Fallback to unfiltered data if filtering results in empty sets
  const finalData = {
    sales: filteredData.sales.length === 0 ? (sales || []) : filteredData.sales,
    purchases: filteredData.purchases.length === 0 ? (purchases || []) : filteredData.purchases,
    salaryPayments: filteredData.salaryPayments.length === 0 ? (salaryPayments || []) : filteredData.salaryPayments,
    workerExpenses: filteredData.workerExpenses.length === 0 ? (workerExpenses || []) : filteredData.workerExpenses,
    checks: filteredData.checks.length === 0 ? (checks || []) : filteredData.checks,
  };
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `${reportTypes.find(r => r.id === selectedReport)?.name || 'Report'} - ${format(new Date(), 'yyyy-MM-dd')}`,
  });
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.setFontSize(10);
      pdf.text(`Generated: ${format(new Date(), 'dd/MM/yyyy HH:mm')}`, 105, 40, { align: 'center' });
      pdf.text(`Period: ${format(new Date(dateRange.startDate), 'dd/MM/yyyy')} - ${format(new Date(dateRange.endDate), 'dd/MM/yyyy')}`, 105, 50, { align: 'center' });
      ...
      pdf.save(`${reportName.replace(/\s+/g, '_')}_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
    } catch (error) {
  ...
  const generateSalesPDF = async (pdf, startY) => {
    try {
      if (typeof pdf.autoTable !== 'function') {
        throw new Error('PDF table generation not available. Please check jsPDF autoTable plugin.');
      }

      const tableData = finalData.sales.map(sale => {
        const customer = customers.find(c => c.id === sale.customerId);
        const customerName = sale.customerName || (customer ? customer.name : 'Walk-in Customer');
  ...
  const generateFinancialPDF = async (pdf, startY) => {
    const totalRevenue = finalData.sales.reduce((sum, sale) => sum + (sale.totalAmount || 0), 0);
    const totalExpenses = finalData.purchases.reduce((sum, purchase) => sum + (purchase.totalAmount || 0), 0);
    const totalSalaries = finalData.salaryPayments.reduce((sum, payment) => sum + (payment.amount || 0), 0);
  ...
  const generatePurchasesPDF = async (pdf, startY) => {
    try {
      ...
      const tableData = finalData.purchases.map(purchase => {
        ...
        const vendorName = purchase.vendorName || (vendor ? vendor.name : 'Unknown Vendor');
  ...
  const generateStockAlertPDF = async (pdf, startY) => {
    try {
      ...
      const lowStockProducts = processedProducts.filter(product => (product.stock || 0) < 10);
  ...
  const generateProfitLossPDF = async (pdf, startY) => {
    const revenue = finalData.sales.reduce((sum, sale) => sum + (sale.totalAmount || 0), 0);
    const cogs = finalData.purchases.reduce((sum, purchase) => sum + (purchase.totalAmount || 0), 0);
    const expenses = finalData.salaryPayments.reduce((sum, payment) => sum + (payment.amount || 0), 0) +
                    finalData.workerExpenses.reduce((sum, expense) => sum + (expense.amount || 0), 0);
  ...
  const renderReport = () => {
    ...
    switch (selectedReport) {
      case 'sales':
        return <SalesReport sales={finalData.sales} {...commonProps} />;
      case 'inventory':
        return <InventoryReport products={processedProducts} {...commonProps} />;
      case 'customers':
        return <CustomerReport customers={customers} sales={finalData.sales} {...commonProps} />;
      case 'vendors':
        return <VendorReport vendors={vendors} purchases={finalData.purchases} {...commonProps} />;
      case 'workers':
        return <WorkerReport workers={workers} salaryPayments={finalData.salaryPayments} workerExpenses={finalData.workerExpenses} {...commonProps} />;
      case 'financial':
        return <FinancialReport sales={finalData.sales} purchases={finalData.purchases} salaryPayments={finalData.salaryPayments} workerExpenses={finalData.workerExpenses} {...commonProps} />;
      case 'purchases':
        return <PurchaseReport purchases={finalData.purchases} {...commonProps} />;
      case 'checks':
        return <ChecksReport checks={finalData.checks} {...commonProps} />;
      case 'stockAlert':
        return <StockAlertReport products={processedProducts} {...commonProps} />;
      case 'profitLoss':
        return <ProfitLossReport sales={finalData.sales} purchases={finalData.purchases} salaryPayments={finalData.salaryPayments} workerExpenses={finalData.workerExpenses} {...commonProps} />;
      default:
  ...
  return (
    <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
      ...
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-white mb-3">Select Report Type</h3>
